...
/// `POST /api/audio/transcribe` → `{ok, transcript, provider}` (web_server.py).
...
/// `POST /api/audio/speak` → `{ok, data_url, mime_type, provider}` (web_server.py).
///
...
extension RestClient {
    ...
    /// Per-request timeout for the audio endpoints (B6 /).
    ///
