...
final class VoiceRecorder {
    ...
    /// transcript and is generation-guarded: it is NOT called if the
    ...
    /// while the transcription round-trip was airborne — before a fix the
    ...
    /// invalidates any in-flight transcribe await.
    func cancel() {
        generation += 1
        meterTask?.cancel()
        meterTask = nil
    ...
    func stopAndTranscribe(rest: RestClient) async -> String? {
        ...
        guard gen == generation else { return nil }  // cancelled mid-read
    ...
    /// race-cancel never inserts text —); otherwise discard via `cancel()`.
