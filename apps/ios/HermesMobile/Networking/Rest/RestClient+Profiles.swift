...
// the upstream rebase — they are ABSENT from a stock gateway backend. This
// extension codes against the pinned upstream shapes
...
extension RestClient {
    ...
    /// working HEAD (`web_server.py`, the desktop profiles-management page,
    /// landed at an earlier revision) and returns `200 {"profiles":[…]}` on the gateway.
    /// Probing it would classify a stock gateway / this-branch backend as
    ...
    /// origin/main `web_server.py`), so THAT is the existence probe.
    ///
    ...
    // web_server.py). Both surface as ``RestError/badStatus`` carrying
    ...
    // PATCH (the `SessionRename` model, web_server.py). A `nil`/empty
