...
/// (upload / devices / approvals / fs / push). The de-patch moved these
...
enum APIPathStyle: String, Sendable, Codable {
    ...
    /// pre-de-patch gateways (e.g. a gateway that has not applied the seam patch yet).
    case legacy
...
struct RestClient: Sendable {
    ...
    /// decoding the `total` field so the drawer's count affordance works.
    ...
    /// Side-effect-free probe of the plugin mount itself: `GET
