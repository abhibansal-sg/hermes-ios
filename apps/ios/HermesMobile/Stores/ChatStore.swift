...
final class ChatStore {
    ...
    ///  the next turn adopted the stale activity.
    ...
    // when a change made unaccepted truncations roll back).
    ...
    /// Never derived from `isStreaming`.
    ...
    /// drain them only into that session.
    ...
    func handle(event: GatewayEvent) {
        ...
        case .messageComplete, .approvalRequest, .clarifyRequest,
             ...
             // `error` ENDS a turn (like message.complete), never begins one.
             .error,
             // session.info carries model/reasoning/fast hot-swap state;
        ...
        case .gatewayReady, .statusUpdate, .unknown,
             // session.info is handled by ConnectionStore, not ChatStore.
             .sessionInfo:
            break
        }
    }
    ...
    /// exception). Before this event was dropped at every routing layer,
    ...
    private func handleGatewayError(_ payload: JSONValue) {
        ...
        // The errored turn's Live Activity dies with it:
    ...
    private func handleForeignFrame(_ event: GatewayEvent) {
        ...
        if event.type == .messageComplete || event.type == .error {
            ...
            let backfillTask = Task {
                ...
                // previously only a LOCAL complete triggered the
        ...
        case .messageStart:
            ...
            // surface the foreign turn's USER prompt NOW. The gateway
        ...
        case .messageComplete, .gatewayReady, .statusUpdate, .unknown,
             ...
             .error,
             // session.info is session-local (config hot-swap feedback),
    ...
    private func beginStreamingMessage(foreign: Bool = false) {
        ...
        // truth for local ownership). This `assert` pins the spec that a
    ...
    private func handleToolComplete(_ payload: ToolCompletePayload) {
        ...
        // (tui_gateway/server.py _on_tool_complete) and also mirrors it to
    ...
    private func handleMessageComplete(_ payload: JSONValue) {
        ...
        mutateStreaming { message in
            ...
            // Authoritative final reasoning (item 5): replaces whatever
            ...
            // Non-success terminal status (item 5): surface it on the
        ...
        // request. Mirrors InboxStore's expire-on-complete. The
    ...
    /// `includeSecure` additionally drops a transient
    ...
    func seedContextUsageFromStatus(runtimeId: String) async {
        ...
        // (item 3) and avoids decoding the unrelated status metadata.
    ...
    func respondSecurePrompt(value: String?) async {
        ...
        pendingSecurePrompt = nil
        guard let client else { return }
        // The gateway's `_respond` router (server.py) routes the reply purely
    ...
    /// index (`prompt.submit`, server.py). The `4018` stale-target path is
    ...
    func restoreCheckpoint(toUserMessageId messageId: UUID) async {
        ...
        // another client was active. If the session IS genuinely busy,
    ...
    /// `_coerce_seed_history` rules (server.py) so the create call accepts
    /// every item:
    ...
    func editAndResend(messageId: UUID, newText: String) async {
        ...
        // Local ownership gate, not `isStreaming` — see restoreCheckpoint.
        guard !localTurnInFlight else {
            lastError = "Agent is busy"
            return
        }
    ...
    private func submitTruncating(
        ...
        // foreign turn that re-adopts during the fetch.
        cancelStreaming()
    ...
    /// the visible stream can never be stopped from this device.
    ...
    /// `tui_gateway/server.py`) — a reply without it 4009s ("no pending
    ...
    /// (server.py). The ``WorkingDirPicker`` (A1) returns a RELATIVE path
    ...
    func seed(normalized: [ChatMessage]) {
        ...
        // open() the session another client is driving, the foreign
    ...
    func backfill() async {
        ...
        do {
            ...
            // a local turn the user just started must not be
            ...
            // against a stale — possibly dead — runtime. The inbox
            ...
            // never surface it on the NEW session ('s catch-side twin).
    ...
    /// surface a foreign-mirrored turn's USER prompt at message.start,
    ...
    func reset() {
        ...
        // state (see ChatView's loading/error split).
        lastBackfillError = nil
    ...
    /// conversation…" spinner. `SessionStore.seedTranscript` calls
    ...
    private func cancelStreaming() {
        ...
        // Live Activity instead of orphaning it. cancelStreaming is
    ...
    /// `guard !isStreaming` (for a local turn, for an adopted
    ...
    func handleConnectionDrop() {
        ...
        // foreign-only drop returns at the guard and would orphan it.
