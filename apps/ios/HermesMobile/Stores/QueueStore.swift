...
final class QueueStore {
    ...
    struct QueuedPrompt: Identifiable, Codable, Equatable, Sendable {
        ...
        /// misrouted destructive prompts). `nil` (including every
    ...
    /// for; pass nil only when no session is active (draft/offline
    ...
    /// Session affinity: a prompt stamped with a `storedSessionId` is
    ...
    /// server had already delivered, i.e. a double-send.
    ...
    ///  against the same failure was guaranteed data loss; the
    ...
    func drain(chat: ChatStore) async {
        ...
        while index < items.count, !chat.isStreaming {
            ...
            // for a session that isn't the active one.
            ...
            // Not accepted: restore the prompt where it was and stop.
