...
final class ServerCapabilities {
    ...
    /// The plugin mount (`/api/plugins/hermes-mobile/…`). Probed
    ...
    /// already exists on the gateway (the desktop profiles page) and would
    /// classify a stock gateway as available, breaking dormancy. EVERY profile
    ...
    /// gateway (a stock gateway) shows none of the switcher chrome and never
    ...
    func probe(serverURL: String, rest: RestClient, force: Bool = false) async {
        ...
        // restore: after the socket actually DROPPED, the same URL may
        ...
        // Stage 1: resolve the plugin mount FIRST — its result selects
    ...
    /// `GET /api/profiles`, which already exists on the gateway). A multi-profile
    ...
    private struct Cache: Codable {
        ...
        init(from decoder: Decoder) throws {
            ...
            // Tolerant so a cache written by a pre- build (no
    ...
    private func persist() {
        ...
        // launches until the URL or app version changes.
