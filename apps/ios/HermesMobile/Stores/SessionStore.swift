...
final class SessionStore {
    ...
    /// the drawer's "N of TOTAL" count affordance (item 6).
    ...
    /// This is the dedicated, observed surface for mutation failures:
    ...
    /// Interrupt seam for the actively-streaming session. In the
    /// app this calls the EXISTING `ChatStore.interrupt()` (which routes to the
    /// STREAM's own runtime) so deleting a live session stops the
    ...
    /// by `(lastActive ?? startedAt) DESC` (desktop parity: item 2).
    ...
    /// sidebar's `workspaceGroupsFor` (the desktop app).
    ///
    ...
    /// **Non-destructive merge (item 3):** after the fetch resolves, rows
    ...
    /// **Stale-token guard (item 3):** a monotonic `refreshToken` is
    ...
    /// Non-destructive merge of an incoming page into `sessions` (item 3,
    ...
    private func mergeSessionPage(_ incoming: [SessionSummary], total: Int?, isAppend: Bool = false) {
        ...
        // First-page (replace) path — original merge semantics.
        ...
        // carry a HIGHER local `lastActive` forward over the incoming
    ...
    /// optimistically bump a session's activity to NOW so a live frame
    ...
    func noteActivity(storedId: String?) {
        ...
        // the optimistic bump and the LIVE WINDOW are the same signal: a
    ...
    /// stale-token guard — so a slow prior response is always discarded.
    func startHeartbeat() {
    ...
    /// coalescing seam (bumps `refreshToken`, discards stale responses) so
    ...
    func open(_ summary: SessionSummary, revealOnFirstPaint: (@MainActor () -> Void)? = nil) {
        ...
        // the existing supersession gate).
        //
        ...
        Task { [weak self] in
            ...
            do {
                ...
                if let chainTip = result.storedSessionId, chainTip != summary.id {
                    ...
                    // outrunnable by a newer open() as the fast path.
    ...
    func createDraftSession() async throws {
        ...
        do {
            ...
            // on the chosen model. Best-effort: a
    ...
    /// path (`server.py`). The gateway's `_coerce_seed_history`
    /// (`server.py`) accepts only `{role ∈ user/assistant/system, non-empty
    ...
    func branchSession(seed: [JSONValue], cwd: String?) async throws
        ...
        do {
            ...
            // and `seed()` rightly refuses to wipe a live mirror —
    ...
    /// fix — three correctness changes over the old swallow-the-throw body:
    ...
    ///  holds open, we (a) interrupt the in-flight turn (— stop an
    ...
    func delete(_ summary: SessionSummary) async {
        ...
        if isActive {
            // stop the actively-streaming runtime before tearing it down.
    ...
    func resumeActiveAfterReconnect() async -> String? {
        ...
        do {
            ...
            // session (the class, on the un-affinity-guarded live path).
    ...
    /// the archive/rename/delete error surface.
    ...
    private func seedTranscriptCacheFirst(
        ...
        do {
            ...
            guard openToken == token else { return }  // superseded
    ...
    /// `token` is the ``openToken`` re-checked AFTER the REST await:
    ...
    /// EXISTING `ChatStore.interrupt()` (routing preserved verbatim).
...
/// for the drawer to present as a system alert (failures used to be
