...
final class LiveActivityManager {
    ...
    /// Tail of the FIFO delivery chain: every ActivityKit content push
    ...
    /// Lifecycle token for the token-registration round-trip: a
    ...
    /// A detached activity awaiting its grace dismissal. `end()` hands
    ...
    func start(sessionTitle: String, sessionId: String? = nil) {
        ...
        // the old reuse path could adopt an activity ActivityKit had
        ...
        do {
            ...
            let started = try Activity.request(
                ...
                content: ActivityContent(
                    state: state,
                    // an orphaned frame self-stales instead of posing
    ...
    /// The manager's state is detached and cleared SYNCHRONOUSLY:
    ...
    func end() {
        ...
        unregisterLAToken()
        // Invalidate any in-flight registration round-trip: its
        ...
        // already in flight) still delivers in order.
    ...
    private func update(phaseOverride: String? = nil) {
        ...
        let content = ActivityContent(
            ...
            // Every live frame refreshes the self-stale horizon.
            staleDate: Date().addingTimeInterval(Self.staleAfter)
        )
    ...
    /// Append a content push to the FIFO delivery chain. Each delivery
    ...
    /// re-checked after: `end()` (or a fresh `start()`) during the
    ...
    private func registerLAToken(_ hex: String) async {
        ...
        guard generation == registrationGeneration else {
            ...
            // its routing. When skipped, the new turn's
