...
/// no label and no capsule (the spec: GLYPH only, `mutedFg`). The drawer is
...
/// One session row in the drawer list. Hierarchy per the spec:
...
struct DrawerSessionRow: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 6) {
                    if let date = summary.displayDate {
                        // tick the relative-time label every 60s so
        ...
        // expose the date + preview as a VoiceOver value so blind users
...
/// session. Theme `midground` per the spec. Animation is local and
