...
/// When `gatewayClient` is non-nil, a "Default Reasoning" and a
...
struct ModelPickerView: View {
    ...
    // global default fast/reasoning state (Settings surface only).
    ...
    var body: some View {
        ...
        PanelContent(phase: phase, label: "Loading models\u{2026}", retry: { Task { await load() } }) { options in
            List {
                ...
                // Global default fast/reasoning controls (Settings surface).
