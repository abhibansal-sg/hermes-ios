...
struct RootView: View {
    ...
    /// so those taps dead-ended silently. A sheet at this root is
    ...
    private var content: some View {
        ...
        case .needsSetup:
            ...
            // QR scan + a slide-up manual ConnectionSetupView fallback.
            // Integrator reconciles if B4 has not landed.
            WelcomeView()
        case .hydrating:
            // a verified connection whose gateway state is still being
        ...
        case .connecting, .reconnecting, .offline:
            // P0 GATE: a `.connecting`/`.offline` phase only
...
private struct SplitLayout: View {
    ...
    private var detailColumn: some View {
        ...
        if sessions.activeStoredId != nil || sessions.isDraft {
            ChatView(
                ...
                // blank transcript (— it was hardcoded to the default
                // false here).
    ...
    private var inspectorColumn: some View {
        NavigationStack {
            ...
            // way back. Snap it home when the tree empties.
...
private struct CompactLayout: View {
    ...
    /// a rightward drag may START an open swipe. Level 02G: expanded to 1.0
    ...
    private func chatStack(safeTop: CGFloat) -> some View {
        // Mirror SplitLayout's detail gate: with no active session
