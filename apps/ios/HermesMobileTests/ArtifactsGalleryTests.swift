...
/// Unit tests for Task #14: artifacts gallery iOS grid + load-more.
///
...
///  10. ``testLoadMoreAppendsPageTwoWithOffset`` — load-more fetches offset=50 and
///  appends new artifacts; `hasMore` reflects remaining count vs total.
///  11. ``testLoadMoreStopsWhenTotalReached`` — `loadMore` no-ops once
///  `artifacts.count >= galleryTotal`.
///  12. ``testFilterChangeResetsOffsetAndResults`` — `load()` clears prior results
...
///  13. ``testNoDuplicateArtifactIdsAcrossPages`` —/cor-A3: cross-page dedup by
...
final class ArtifactsGalleryTests: XCTestCase {
    ...
    func testSameUrlTwiceInOneMessageGetsDistinctIds() async {
        ...
        let dummyClient = RestClient(baseURL: URL(string: "http://127.0.0.1:8080")!, token: "t")
    ...
    // MARK: - 10. load-more appends page 2 with correct offset
    ...
    func testLoadMoreAppendsPageTwoWithOffset() async {
        ...
        let dummyClient = RestClient(baseURL: URL(string: "http://127.0.0.1:8080")!,
                                     token: "t")
    ...
    // MARK: - 11. loadMore no-ops when total reached
    ...
    // MARK: - 12. filter change resets offset and results
    ...
    // MARK: - 13. no duplicate artifact ids across pages
