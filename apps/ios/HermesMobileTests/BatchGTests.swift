...
/// (R1 Batch G, iOS half) — connection lifecycle and diagnostics.
///
...
final class BatchGTests: XCTestCase {
    ...
    func testForcedProbeBypassesDiskCacheAfterReconnect() async {
        ...
        // regression re-verification: the entirely-INCONCLUSIVE forced probe must not have
    ...
    /// carrying the gateway's chat-disabled reason (the post-server
