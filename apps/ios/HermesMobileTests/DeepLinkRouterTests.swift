...
final class DeepLinkRouterTests: XCTestCase {
    ...
    func testPairOnUnconfiguredDoesNotRequestConfirmation() {
        ...
        HermesURLRouter.route(
            URL(string: "hermesapp://pair?url=https%3A%2F%2Fh%3A8080&token=tok")!,
            connection: s.connection,
            sessions: s.sessions,
            chat: s.chat,
            inbox: s.inbox,
    ...
    func testMalformedPairIsIgnoredEvenUnconfigured() {
        ...
        HermesURLRouter.route(
            URL(string: "hermesapp://pair?url=https%3A%2F%2Fh%3A8080")!,
            connection: s.connection,
            sessions: s.sessions,
            chat: s.chat,
            inbox: s.inbox,
    ...
    func testDeepLinkCoordinatorStashAndClear() {
        ...
        let first = HermesURLRouter.PairPayload(
            url: "https://a:8080", token: "t1", isDeviceToken: false, deviceId: nil, manualToken: false
        )
        ...
        let second = HermesURLRouter.PairPayload(
            url: "https://b:8080", token: "t2", isDeviceToken: false, deviceId: nil, manualToken: false
        )
    ...
    func testManualTokenPairPayloadParsesWithNoToken() {
        let payload = "hermesapp://pair?url=http%3A%2F%2F127.0.0.1%3A8080&manual_token=true"
        ...
        XCTAssertEqual(parsed?.url, "http://127.0.0.1:8080")
    ...
    func testStandardPairPayloadHasManualTokenFalse() {
        let payload = "hermesapp://pair?url=https%3A%2F%2Fh%3A8080&token=tok"
        let parsed = HermesURLRouter.parsePairPayload(payload)
    ...
    func testManualTokenPairRouteCallsManualTokenSeam() {
        ...
        HermesURLRouter.route(
            URL(string: "hermesapp://pair?url=http%3A%2F%2F127.0.0.1%3A8080&manual_token=true")!,
            connection: s.connection,
            sessions: s.sessions,
            chat: s.chat,
            inbox: s.inbox,
    ...
    func testDeepLinkCoordinatorManualTokenStashAndClear() {
        ...
        let payload = HermesURLRouter.PairPayload(
            url: "http://127.0.0.1:8080", token: "", isDeviceToken: false, deviceId: nil, manualToken: true
        )
        coordinator.requestManualTokenPair(payload)
        XCTAssertNotNil(coordinator.pendingManualTokenPair)
        XCTAssertEqual(coordinator.pendingManualTokenPair?.url, "http://127.0.0.1:8080")
