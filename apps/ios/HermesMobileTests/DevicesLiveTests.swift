...
/// `RestClient` layer (NOT the UI) §VERIFY.
///
...
/// points at. Per the spec it must ONLY be run against an own throwaway
/// instance (never a stock gateway / shared backend).
final class DevicesLiveTests: XCTestCase {
    ...
    private func liveClient() async throws -> RestClient {
        ...
        // Mirror production: probe the plugin mount once and pin the
