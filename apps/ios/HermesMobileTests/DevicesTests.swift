...
/// from the pinned shapes §Interface), the QR v1/v2 parse
...
final class DevicesTests: XCTestCase {
    ...
    func testQRv1ParsesAsSharedPairing() {
        ...
        let payload = "hermesapp://pair?url=https%3A%2F%2Fhost%3A8080&token=sharedtok123"
        let parsed = HermesURLRouter.parsePairPayload(payload)
        XCTAssertNotNil(parsed)
        XCTAssertEqual(parsed?.url, "https://host:8080")
    ...
    func testQRv2ParsesDeviceTokenAndRecordsId() {
        ...
        let payload = "hermesapp://pair?url=https%3A%2F%2Fhost%3A8080&token=devtok456&kind=device&device_id=dev_qr789"
        let parsed = HermesURLRouter.parsePairPayload(payload)
        XCTAssertNotNil(parsed)
    ...
    func testRecordedDeviceIdPersistencePerServer() {
        ...
        let serverA = "https://a:8080"
        let serverB = "https://b:8080"
    ...
    func testDeviceNameHintIsNonEmpty() {
        ...
        // model name acceptable per the spec's open question).
