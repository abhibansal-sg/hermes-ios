...
/// gate (live dashboard on a test gateway)
...
final class PushEventPrefsTests: XCTestCase {
    ...
    func testListOrderIsStable() {
        // Order is part of the spec for deterministic access-log assertions.
...
// MARK: - Push-token registration dedupe: env key
...
final class PushRegistrarEnvDedupeTests: XCTestCase {
    ...
    func testEnvKeyAbsentCausesDedupeMiss() {
        // Simulate the state written by a pre- install: token + events
