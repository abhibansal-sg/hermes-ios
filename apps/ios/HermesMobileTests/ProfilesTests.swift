...
/// the design contract §Interface), the `SessionSummary.profile` round-trip (incl. the
...
final class ProfilesTests: XCTestCase {
    ...
    func testProbeClassificationByStatusAndBody() {
        ...
        // already exists on the gateway and would break dormancy). Pin the
    ...
    func testProfileSummaryDecodesDefaultAndNamedRows() throws {
        // Captured verbatim from `_profile_to_dict` (web_server.py): the
    ...
    func testProfilesSessionsResultDecodesWrapperAndTagsEachRow() throws {
        // Captured verbatim from web_server.py + per-row tags
    ...
    func testSwitcherVisibilityGate() {
        ...
        // (unavailable, *) ⇒ hidden (stock gateway — a stock gateway)
    ...
    func testStrictUnknownProfileErrorsSurfaceReadableMessages() {
        ...
        // (web_server.py) and `400` on an invalid name. Both arrive as
