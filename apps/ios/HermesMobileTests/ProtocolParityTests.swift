...
/// gateway/iOS protocol-parity follow-ons. Every fixture below mirrors
...
final class ProtocolParityTests: XCTestCase {
    ...
    func testErrorEventDecodes() throws {
        // server.py — _emit("error", sid, {"message": "agent init failed: …"})
    ...
    func testClarifyRequestIdDecodes() {
        // _block injects request_id into every clarify frame (server.py).
    ...
    func testSessionOpenResultFallsBackToResumed() throws {
        ...
        // (server.py), NOT `stored_session_id`.
    ...
    /// Batch D / §3.2 (fixes D8): a turn with text-interleaved single-tool
    ...
    /// which asserted the turn-total collapse-all behavior the spec names as
    /// the D8 defect.)
    ...
    /// Batch D / §3.2: consecutive tools with NO prose between them form
    ...
    /// §2.1: a SEEDED message materializes its scalar seed content into
