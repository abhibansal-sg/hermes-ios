...
/// (UX-1 P0) — session delete-flow correctness + failure surface.
///
...
///  1. interrupt the in-flight turn → `session.close` (RUNTIME id) →
...
final class SessionDeleteFlowTests: XCTestCase {
    ...
    /// `interruptActive` so a single call sequence proves the ordering.
    ...
    // MARK: - Failure path (the core surface)
