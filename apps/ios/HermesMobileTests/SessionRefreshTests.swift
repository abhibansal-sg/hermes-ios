...
/// Unit tests for desktop-parity session list refresh.
///
...
final class SessionRefreshTests: XCTestCase {
    ...
    // MARK: - stale device-clock bump must decay (sort + timestamp)
    ...
    /// This guards the anti-flicker behavior the change must not regress.
    func testLiveBumpStillCarriedForwardToPreventFlicker() async {
        ...
        // carry-forward now gates on the explicit turn-in-progress flag.
...
// MARK: - explicit turnInProgress carry-forward gate
