...
/// regression test: ``SessionStore/restAPI`` must track the live
...
final class SessionStoreTokenSourceTests: XCTestCase {
    ...
    private let liveToken  = "live-token"
    private let staleToken = "stale-token"
    ...
    // MARK: - single token-source invariant
    ...
    ///  `token = KeychainService.loadToken(server: urlString) // "stale-token"`
    ///  -> `restAPITokenForTesting` would equal `staleToken` -> XCTAssertEqual FAILS.
    ///

    /// Post-fix ``restAPI`` body:
    ///  `connection?.rest` // token sourced from `currentToken` = "live-token"
    ...
    func testRestAPIUsesLiveConnectionTokenNotStaleKeychain() async {
        ...
        XCTAssertEqual(
            ...
            "currentToken, not the stale Keychain value")
        XCTAssertNotEqual(
            sessions.restAPITokenForTesting, staleToken,
            "restAPI must NOT use the stale Keychain token")
    }
