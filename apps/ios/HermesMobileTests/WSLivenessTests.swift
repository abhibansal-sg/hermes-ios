...
/// + Inc-1/Inc-3 — WS liveness ping, orphaned-LA reconcile,
...
///  the rolling-window safety invariant.
@MainActor
final class WSLivenessTests: XCTestCase {
    ...
    // MARK: - (g) WIRING: clearAllTurnsInProgress is called by both abandon paths
    ...
    /// session list into a never-converge loop.
    ///
    ...
    func testDeadSocketBranchClearsTurnsInProgress() async {
        ...
        XCTAssertTrue(sessions.turnsInProgressIds.isEmpty,
            "handleScenePhase dead branch must clear turnsInProgress")
    }

    /// `handleScenePhase(.active)` with `probeLivenessRPC` returning `false`
    /// (the dead-probe branch) must also clear `turnsInProgress`.
    ///
    ...
    func testDeadProbeBranchClearsTurnsInProgress() async {
        ...
        XCTAssertTrue(sessions.turnsInProgressIds.isEmpty,
            "handleScenePhase dead-probe branch must clear turnsInProgress")
    ...
    /// This test:
    ///  1. Asserts the constant equals 5 min.
