...
/// Spec (the design contract-MODES.md §Inc2):
...
final class WSURLBuilderTests: XCTestCase {
    ...
    func testEffectiveHost_sharedDashboard_alwaysLoopback() {
        let nonLoopback = URL(string: "http://192.168.1.100:8080")!
    ...
    func testEffectiveHost_localDesktop_alwaysLoopback() {
        let nonLoopback = URL(string: "http://10.0.0.5:8080")!
    ...
    func testEffectiveHost_remoteURL_nonLoopbackIP_omitsOverride() {
        let remote = URL(string: "http://192.168.1.42:8080")!
    ...
    func testEffectiveHost_remoteURL_hostnameTarget_omitsOverride() {
        let remote = URL(string: "http://mymac.local:8080")!
        let host = WSURLBuilder.effectiveHost(for: remote, mode: .remoteURL)
        XCTAssertNil(
            host,
    ...
    func testWsRequest_sharedDashboard_nonLoopback_hasLoopbackHost() {
        let req = wsRequest(urlString: "http://192.168.1.42:8080", mode: .sharedDashboard)
        XCTAssertEqual(
    ...
    func testWsRequest_remoteURL_nonLoopback_hasRealHost() {
        ...
        let req = wsRequest(urlString: "http://192.168.1.42:8080", mode: .remoteURL)
    ...
    func testWsRequest_remoteURL_loopbackTarget_hasLoopbackHost() {
        ...
        let req = wsRequest(urlString: "http://127.0.0.1:8080", mode: .remoteURL)
        XCTAssertEqual(
            req.value(forHTTPHeaderField: "Host"), "127.0.0.1",
    ...
    func testRestRequest_sharedDashboard_nonLoopback_hasLoopbackHost() {
        let req = restRequest(urlString: "http://192.168.1.42:8080", mode: .sharedDashboard)
        XCTAssertEqual(
            req.value(forHTTPHeaderField: "Host"), "127.0.0.1",
    ...
    func testRestRequest_remoteURL_nonLoopback_omitsLoopbackHost() {
        let req = restRequest(urlString: "http://192.168.1.42:8080", mode: .remoteURL)
        XCTAssertNotEqual(
            req.value(forHTTPHeaderField: "Host"), "127.0.0.1",
    ...
    func testSafeForManualTokenPair_privateURLs_accepted() {
        XCTAssertTrue(WSURLBuilder.isSafeForManualTokenPair("http://127.0.0.1:8080"))
        XCTAssertTrue(WSURLBuilder.isSafeForManualTokenPair("http://192.168.1.42:8080"))
        XCTAssertTrue(WSURLBuilder.isSafeForManualTokenPair("http://10.0.0.5:8080"))
        XCTAssertTrue(WSURLBuilder.isSafeForManualTokenPair("http://mymac.local:8080"))
        XCTAssertTrue(WSURLBuilder.isSafeForManualTokenPair("http://localhost:8080"))
        // Tailscale MagicDNS — the stable-address path from lane 4a.
        XCTAssertTrue(WSURLBuilder.isSafeForManualTokenPair("http://mymac.tailnet.ts.net:8080"))
    }

    func testSafeForManualTokenPair_publicURLs_rejected() {
        XCTAssertFalse(WSURLBuilder.isSafeForManualTokenPair("http://8.8.8.8:8080"))
        XCTAssertFalse(WSURLBuilder.isSafeForManualTokenPair("http://myserver.example.com:8080"))
        ...
        XCTAssertFalse(WSURLBuilder.isSafeForManualTokenPair("http://134744072:8080"),
                       "integer-IP URL for 8.8.8.8 must be rejected")
        XCTAssertFalse(WSURLBuilder.isSafeForManualTokenPair("http://16843009:8080"),
