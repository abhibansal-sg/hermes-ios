...
verbatim from ``hermes_cli/web_server.py`` in the de-patch (W1):
...
class FsSandboxError(Exception):
    ...
    sandbox/lookup failures into the spec-pinned responses without leaking a
...
def _resolve_under_session_cwd(session_id: str, rel_path: str) -> Tuple[str, str]:
    ...
    the spec status/body when the target escapes the root.
