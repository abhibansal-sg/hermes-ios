...
class _DesktopGatewayResult:
    ...
    ----------
    url : str
        The discovered gateway base URL (e.g. ``http://127.0.0.1:8080`` or a
        remote HTTPS URL).
    token : str or None
...
# canonical path after the de-patch; the legacy top-level path keeps
# pairing working against servers that predate it (e.g. the gateway
# until it applies the seam patch).
_DEVICE_ISSUE_PATHS = (
