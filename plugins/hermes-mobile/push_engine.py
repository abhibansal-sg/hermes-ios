...
Moved verbatim from ``hermes_cli/push_notify.py`` plus
...
emit-observer seam, wired by :func:`activate`.
...
# Per the spec: the Live Activity topic is the bundle id suffixed with the
...
# The gateway's ``_emit`` / finalize / interrupt paths
...
def handle_gateway_event(event: str, sid: str, payload: dict | None = None) -> None:
    """S2 emit-observer entry point seam S2).
